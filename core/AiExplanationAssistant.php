 ...
 * AI provider's chat-completions endpoint using one of the admin's saved
 * prompt "tabs" (see AiSettings::$prompts), and returns the AI's response
 ...
 *
 * Uses cURL rather than file_get_contents(): some hosts block outbound
 * file_get_contents() over HTTPS, or have allow_url_fopen disabled, while
 * cURL still works. Falls back to file_get_contents() if cURL isn't
 * available at all.
 */
class AiExplanationAssistant
{
     ...
     * @param string $promptId Which saved prompt tab to use. Empty/unmatched falls back to the first configured prompt.
     * @return array ['ok' => bool, 'text' => string|null, 'error' => string|null]
     */
    public static function improveExplanation(array $question, string $promptId = ''): array
    {
        ...
        $promptRow = AiSettings::findPrompt($promptId);
        if (!$promptRow || trim((string)($promptRow['prompt'] ?? '')) === '') {
            return ['ok' => false, 'text' => null, 'error' => 'No prompt template configured. Set one in Admin → AI Settings.'];
        }

        $prompt = self::fillTemplate((string)$promptRow['prompt'], $question);
