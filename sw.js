/* ============================================================
   ...
   Mostly no-cache / network-only, EXCEPT for a small, explicit
   list of routes (OFFLINE_ROUTES below) that get cached so the
   app still opens when the user is offline.

   Everything not in that list — other pages, images, API calls —
   passes straight through to the network, uncached, same as before.
   ============================================================ */

const CACHE_VERSION = 'qz-offline-v1'; // bump this string to force an update
const OFFLINE_CACHE = `qz-offline-${CACHE_VERSION}`;
const OFFLINE_FALLBACK = 'pwa/offline.html';

/* ── Edit this list to control exactly what works offline ─── */
...
/* ── Install: cache only the routes above, then activate ──── */
      ...
      await Promise.allSettled(
        OFFLINE_ROUTES.map(async url => {
          try {
            ...
            console.warn('[SW] Failed to cache offline route:', url, err);
          }
        })
      );
...
/* ── Fetch: only the listed routes touch the cache ─────────── */
self.addEventListener('fetch', event => {
  const { request } = event;
  if (request.method !== 'GET') return;

  const url = new URL(request.url);
  const path = url.pathname.replace(/^\//, ''); // strip leading slash for comparison

  const isOfflineRoute = OFFLINE_ROUTES.includes(path);

  if (isOfflineRoute) {
    event.respondWith(networkFirstThenCache(request, path));
    return;
  }

  // Navigations that aren't in the offline list: try network,
  // fall back to the offline page if there's no connection.
  ...
  // Everything else: pure network passthrough, no caching at all.
  event.respondWith(fetch(request));
});

/* Network-first for offline-listed routes, updating the cache
   on every successful fetch, falling back to cache when offline */
...
/* ── Skip waiting on message (used by pwa.js update flow) ─── */
...
console.log('[SW] Service worker initialized — offline routes:', OFFLINE_ROUTES);
